...
// Proxy browser tool calls to internal Neuron service.
// Used by npx clients that default ZENROWS_BROWSER_URL to https://mcp.zenrows.com/browser.
...
app.all("/browser/*", async (c) => {
  ...
  const target = `${BROWSER_URL.replace(/\/$/, "")}${path}`;
